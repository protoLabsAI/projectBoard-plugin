...
- **Escalation (opt-in)** — with a `coders` map of >1 distinct delegate, a capability
  ...
  client too old; #420) rotates the same way, and is skipped by later cards for 30
  minutes. Climbing a rung means "a stronger model may succeed"; rotating within one
  means "this model is fine, its provider is not" — the two never mix.
