...
| `POST` | `/features/{fid}/salvage` | Publish a stranded card's worktree WITHOUT dispatching a coder (#427): commit what the tree holds, run the pre-PR gate, push, open the PR, move the card to `in_review`. Body: `{force?: bool, tree?: str}`. Only for a stranded card (`in_progress` with no live drive, or `blocked`); refuses (409, nothing changed) while a drive owns it, or when no worktree — or more than one, unless `tree` names one — has changes vs base. A red gate publishes nothing (409 `gate-red`, with the output's tail) unless `force: true`, which opens a DRAFT PR carrying that output. 200 `published` · 404 `not-found` · 503 `loop-not-running` · 502 `error`. |
